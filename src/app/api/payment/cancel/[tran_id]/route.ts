...
export async function POST(req: NextRequest, { params }: { params: { tran_id: string } }) {
  ...
  try {
    ...
    if (!querySnapshot.empty) {
      ...
      await updateDoc(transactionDoc.ref, { status: 'cancelled' });
    }
  } catch (error) {
    console.error("Failed to update transaction status to 'cancelled':", error);
  }

  const clientUrl = process.env.NEXT_PUBLIC_CLIENT_URL || 'http://localhost:9002';
  return NextResponse.redirect(`${clientUrl}/payment/cancel`);
}
